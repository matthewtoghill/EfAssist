...
**A core settings file plus one file per workspace.** `%AppData%/EfMigrateHub/settings.json` holds app-wide preferences and the recent list; each workspace's choices live in `workspaces/<name>-<hash>.json`, named for the solution and hashed on the absolute path so same-named solutions in different repos can't collide. Workspace files are read lazily, on open, so startup cost doesn't grow with the number of workspaces ever used.
...
EfMigrateHub.slnx
src/
  EfMigrateHub.Core/          class library — no UI reference
  ...
  EfMigrateHub.App/           Avalonia + CommunityToolkit.Mvvm
tests/
  EfMigrateHub.Core.Tests/    xunit — parsing + arg-building only
```
